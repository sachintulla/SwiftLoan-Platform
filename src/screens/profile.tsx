...
export default function Profile() {
  ...
  const saveProfile = async () => {
    ...
    showToast(t.tSaved);
    if (isAuthed()) {
      await api.updateProfile({ fullName: state.pdName, email: state.pdEmail }).catch(() => {});
    }
  };
  const changeLang = (code: string) => {
    set({ lang: code });
    if (isAuthed()) api.setLanguage(code).catch(() => {});
  };
  const changeNotif = (patch: Partial<{ loan: boolean; security: boolean; promo: boolean }>, on: boolean) => {
    ...
    if (isAuthed()) {
      ...
      api.setNotifications(map).catch(() => {});
    }
  };
  const logout = async () => {
    ...
    <Screen scroll bottomNav padded>
      ...
      <View style={styles.card}>
        <View style={{ flexDirection: 'row', alignItems: 'center', gap: 14 }}>
          <View style={styles.avatar}><Text style={[font(700), { color: '#fff', fontSize: 18 }]}>{initials(state.pdName)}</Text></View>
          <View style={{ flex: 1 }}>
        ...
        <View style={styles.statsRow}>
          <View style={{ flex: 1 }}>
            ...
            <Text style={[font(800), { fontSize: 20, color: colors.text }]}>780<Text style={[font(500), { fontSize: 12, color: colors.muted }]}> / 900</Text></Text>
          </View>
          <View style={styles.statDiv} />
          <View style={{ flex: 1, flexDirection: 'row', alignItems: 'center', gap: 6 }}>
            <Icon name="event_available" size={18} color={colors.mint} />
            <Text style={[font(600), { fontSize: 12, color: colors.textMid }]}>{t.statOnTimeVal} {t.onTimeRepay}</Text>
          </View>
        </View>
      </View>
      ...
      <SectionCard>
        ...
        {!state.pdEdit ? (
          <View style={{ marginTop: 12 }}>
            ...
            <DetailRow label={t.dob} value={t.dobVal} last />
          </View>
        ) : (
