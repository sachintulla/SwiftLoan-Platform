...
import { saveTokens, clearTokens, clearOffersCache, clearPrefillDraft } from '../state/session';
...
export const api = {
  ...
  verifyOtp: async (phone: string, code: string): Promise<AuthResult> => {
    ...
    // account's name/DOB/address/income as if they belonged to this user.
  ...
  logout: async () => {
    ...
    // Same for the voice agent's saved applicant-details draft.
    await clearOffersCache().catch(() => {});
    await clearPrefillDraft().catch(() => {});
  },
